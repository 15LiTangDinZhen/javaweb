...
<%
        ...
        if (identity.equals("超级管理员")){
            try {
                ...
                String sql = "select * from manager where man_name='" + username + "' and man_pwd = '" + password + "'";//定义SQL语句
    ...
    <%
                ...
                e.printStackTrace();
                    ...
                    try {
                        Connection conn = DBCon.getCon(); // 加载驱动并建立数据库连接
                        Statement stmt = conn.createStatement ();//创建Statement对象
                        String sql = "select * from room where room_people='" + username + "' and room_tel = '" + password + "'";//定义SQL语句
                        ResultSet rs = stmt.executeQuery(sql); //执行SQL语句，返回查询结果集
                        if(rs.next()){ //如果记录集非空，表明有匹配的用户名和密码，登陆成功
